...
function log_debug() {
    ...
    [ "$level_num" -le 1 ] && echo -e "\033[90m" "${content}" "\033[0m"
}
...
function log_info() {
    ...
    [ "$LOG_FLAG" == "true" ] && echo "$content" >>"$LOG_FILE"
    [ "$level_num" -le 2 ] && echo -e "\033[32m" "${content}" "\033[0m"
}
...
function log_warn() {
    ...
    [ "$LOG_FLAG" == "true" ] && echo "$content" >>"$LOG_FILE"
    [ "$level_num" -le 3 ] && echo -e "\033[33m" "${content}" "\033[0m"
}
...
function log_err() {
    ...
    [ "$LOG_FLAG" == "true" ] && echo "$content" >>"$LOG_FILE"
    [ "$level_num" -le 4 ] && echo -e "\033[31m" "${content}" "\033[0m"
}
...
function log_result() {
    ...
    [ "$LOG_FLAG" == "true" ] && echo "$content" >>"$LOG_FILE"
    [ "$level_num" -le 5 ] && echo -e "\033[36m" "${content}" "\033[0m"
}
